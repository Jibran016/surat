...
@section('title', 'Tambah Divisi - Surat Menyurat')
...
<div class="mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-6">
    <h1 class="inline-flex items-center gap-2 text-2xl font-bold text-slate-900">
        <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14"></path><path d="M5 12h14"></path></svg>
        Tambah Divisi
    </h1>
    <p class="mt-1 text-sm text-slate-500">Isi nama divisi baru.</p>

    <form method="POST" action="{{ route('divisions.store') }}" class="mt-5 space-y-4">
        @csrf
        <div>
            <label for="name" class="text-sm font-semibold text-slate-700">Nama Divisi</label>
            <input id="name" name="name" type="text" value="{{ old('name') }}" required class="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500">
